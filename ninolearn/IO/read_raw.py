...
def ZC_raw(version = 'default', spatialres = 'default'):
    ...
    NOTE: Only works for a specific grid in ZC
    """
    print('Processing raw ZC data...')
    ...
    if any(data.dtypes == 'O'):
        print('WARNING: mixed types present, ZC numerical stabillity unlikely for used parameters')
    ...
    if exists(join(processeddir, f'sst_ZC_{version}.nc')):
        pass
    else:
        sst.to_netcdf(join(processeddir, f'sst_ZC_{version}.nc')) 
    
def ZC_raw_lr(version = 'default'):

    """ 
    Loads data as produced by the Zebiak Cane model (fort.149 output) and 
    unpacks the .csv into a 3D grid with dimension 3 being time, also regrids to
    1x1 degree and exports h and sst fields. Version: low resolution output
    """
...
def ZC_h(version = 'default'):
    ...
    '''
    print('calculating thermocline height fields...')
    if version == 'default': print('must specify version!')
